...
function display_gitea_routes_credentails() {
    if [[ $deployment_type == "demo" ]]; then
      ...
      echo "================"
      echo
      echo -n "Default Gitea username: "; oc get secret ${metadata_name}-gitea-secret -o jsonpath='{ .data.gitea_user_name}' | base64 -d; echo
      echo -n "Default Gitea password: "; oc get secret ${metadata_name}-gitea-secret -o jsonpath='{ .data.gitea_user_password}' | base64 -d; echo
    fi
}
...
function display_workflow_workstreams_routes_credentials() {
    ...
    if [[ $item == "workflow" || $item == "document_processing" ]]; then
    ...
    echo
    if [[ $item == "workflow" || $item == "workflow-workstreams" || $item == "document_processing" ]]; then
      echo -e "\x1B[1mYou can access Business Automation Workflow Portal, Case Client, and IBM Workplace using the following URLs:\x1B[0m"
    fi
    if [[ $item == "workstreams" ]]; then
      echo -e "\x1B[1mYou can access Business Automation Workstreams using the following URLs:\x1B[0m"
    fi
    if [[ $item == "workflow" || $item == "workflow-workstreams" || $item == "document_processing" ]]; then
    ...
    echo -e "https://$(oc get routes --no-headers | grep ae-service | awk {'print $2'})"
    ...
    if [[ $deployment_type == "demo" ]]; then
      ...
      echo -n "Default administrator password: "; pwd=$(echo $(oc get secret ${metadata_name}-openldap-customldif -o yaml | grep ldap_user.ldif | cut -d ' ' -f4) | base64 -d | grep "userpassword: " | head -n1); pwd=${pwd//*userpassword: /}; echo "$pwd"
    fi
}
...
function display_contentanalyzer_routes_credentials() {
    ...
    echo
    echo
    echo -e "\x1B[1mYou can access Automation Content Analyzer using the following URLs:\x1B[0m"
...
function display_bai_routes_credentials() {
    ...
    if [[ $deployment_type == "demo" ]]; then
      ...
      echo -n "Default password: "; pwd=$(echo $(oc get secret ${metadata_name}-openldap-customldif -o yaml | grep ldap_user.ldif | cut -d ' ' -f4) | base64 -d | grep "userpassword: " | head -n2); pwd=${pwd//*userpassword: /}; echo "$pwd"
      echo
    fi
...
function display_workflow_authoring_routes_credentials() {
    echo "Below are the available routes for Business Automation Workflow Authoring:"
    ...
    if [[ $item == "workflow" ]]; then
      echo -e "\x1B[1mYou can access Business Automation Studio, Business Automation Workflow Portal, Case Client using the following URLs:\x1B[0m"
    fi
    if [[ $item == "workflow" || $item == "workflow-workstreams" ]]; then
      echo -e "https://$(oc get routes --no-headers | grep bastudio-service | awk {'print $2'})/BAStudio"
      echo -e "https://$(oc get routes --no-headers | grep baw-server | awk {'print $2'})/ProcessPortal"
      echo -e "https://$(oc get routes --no-headers | grep navigator-route | awk {'print $2'})/navigator?desktop=baw"
    fi
    if [[ $item == "workflow-workstreams" ]]; then
...
function display_document_processing_routes_credentials() {
    echo "Below are the available routes for IBM Automation Document Processing:"
