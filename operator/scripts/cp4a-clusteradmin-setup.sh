...
function select_all_namespace(){
    ...
    while true; do
        if [ -z "$CP4BA_AUTO_ALL_NAMESPACES" ]; then
            printf "\x1B[1mDo you want CP4BA Operator support 'All Namespaces'? (Yes/No, default: No) \x1B[0m"
            ...
            *)
            ...
            printf "\x1B[1mDo you want CP4BA Operator support 'All Namespaces'? (Yes/No, default: No)  \x1B[0m$CP4BA_AUTO_ALL_NAMESPACES\n"
...
function verify_existing_csv(){
    ...
    if [ "${#exist_csv_project_array[@]}" -eq "0" ]; then
        printf "\n"
        echo -e "\x1B[1mThe Cloud Pak for Business Automation Operator (Pod, CSV, Subscription) not found in cluster\x1B[0m\nContinue....\n"
    else
        if [[ !(" ${exist_csv_project_array[@]} " =~ "${project_name}") && !(" ${exist_csv_project_array[@]} " =~ "${PROJ_NAME_ALL_NAMESPACE}") && "${ALL_NAMESPACE}" == "No" ]] ; then
            printf "\n"
            echo -e "\x1B[1;31mFound the existing Cloud Pak for Business Automation Operator (Pod, CSV, Subscription) in different project \"${exist_csv_project_array[*]}\"! \x1B[0m\n"

            while true; do
                printf "\x1B[1mDo you want to deploy another CP4BA Operator in new project \"${project_name}\"? (Yes/No, default: No) \x1B[0m"
                read -rp "" ans
                case "$ans" in
                "y"|"Y"|"yes"|"Yes"|"YES")
                    echo -e "Continue....\n"
            ...
            printf "\n"
            echo -e "\x1B[1;31mFound the existing Cloud Pak for Business Automation Operator in \"${PROJ_NAME_ALL_NAMESPACE}\", it already supports All Namespaces! \x1B[0m\nExit..."
            exit 1
...
function prepare_olm_install() {
   ...
   for ((retry=0;retry<=${maxRetry};retry++)); do        
      ...
      isReady=$(${CLI_CMD} get pod -n "$temp_project_name" --no-headers | grep ibm-cp4a-operator | grep "Running")
      if [[ -z $isReady ]]; then
        if [[ $retry -eq ${maxRetry} ]]; then 
...
function create_secret_entitlement_registry(){
    ...
    if [[ "${ALL_NAMESPACE}" == "Yes" ]]; then
        ...
        printf "\x1B[1mCreating docker-registry secret for Entitlement Registry key in project $PROJ_NAME_ALL_NAMESPACE...\n\x1B[0m"  
...
function verify_silence_install(){
    if [[ ! -z "${CP4BA_AUTO_PLATFORM}" || ! -z "${CP4BA_AUTO_DEPLOYMENT_TYPE}" ]]; then
        ...
        if [[ ! " ${deployment_type_array[@]} " =~ " ${CP4BA_AUTO_DEPLOYMENT_TYPE} " ]]; then
            echo -e "\x1B[1;31mOnly \"starter\" or \"custom\" is valid value for environment variable [CP4BA_AUTO_DEPLOYMENT_TYPE].\n\x1B[0m"
            exit 1
        fi
    fi

    if [[ "$CP4BA_AUTO_PLATFORM " == "other" && "$CP4BA_AUTO_DEPLOYMENT_TYPE" == "starter" ]]; then
        echo -e "\x1B[1;31mOnly \"custom\" is valid value for environment variable [CP4BA_AUTO_DEPLOYMENT_TYPE] when [CP4BA_AUTO_PLATFORM] set as \"other\" .\n\x1B[0m"
        exit 1        
    fi
